...
import type { RecentMLAnalysis } from '@/types/monitoring'

defineProps<{
  data: RecentMLAnalysis[]
}>()
...
function getStatusBadge(status: string) {
  switch (status) {
    ...
    case 'timeout':
      return { color: '#F97316', label: 'Timeout' }
    case 'error':
...
<template>
  <div class="card">
    <h3 class="section-title">Recent ML Analyses</h3>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="row in data" :key="row.time">
            ...
            <td class="score">{{ formatScore(row.ml_score) }}</td>
            <td class="score">{{ formatScore(row.final_score) }}</td>
            <td class="confidence">{{ formatConfidence(row.confidence) }}</td>
            <td class="latency">{{ formatLatency(row.latency_ms) }}</td>
            <td class="status">
              <span class="badge" :style="{ color: getStatusBadge(row.status).color }">
    ...
    <div v-else class="empty-state">No recent ML analyses</div>
  </div>
</template>
